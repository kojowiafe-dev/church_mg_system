...
const ForgotPassword = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      console.log('Sending forgot password request with data:', { email: data.email });
      ...
      if (error.response) {
        if (error.response.status === 404) {
          ...
          errorMessage = error.response.data.detail || "Failed to process request";
        }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      <div className="relative z-20 w-full max-w-md bg-white/80 backdrop-blur-sm shadow-lg rounded-lg p-8">
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit(onSubmitVerification)} className="space-y-6">
            <div className="space-y-2">
